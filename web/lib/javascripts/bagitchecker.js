			...
			if (folderpath == '') {
				...
				$.post(url,data,function(response) {
					...
					if(response && response.length > 0) {
						...
						$('#message div').hide().html('<div class="alert alert-warning">There ' + fileslength + ' ' + response.length + ' required missing ' + wording + '<br> <ul id="list-missing"></ul></div>').slideDown(200);

						for (var i = 0; i < response.length; i++) {
							$('#list-missing').append('<li>' + response[i] + '</li>');
						}
